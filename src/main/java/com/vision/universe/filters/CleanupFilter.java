...
public class CleanupFilter implements ImageFilter {
	...
	public CleanupFilter(double percent) {
		this.percent = percent;
	}
	
	public CleanupFilter(double percent,boolean checking) {
		this.percent = percent;
		this.checking = checking;
	}
	
	public CleanupFilter(double percent,boolean checking,int rad) {
		this.percent = percent;
		this.checking = checking;
		this.rad = rad;
	}
	
	public CleanupFilter(double percent,boolean checking,boolean arcOnly) {
		this.percent = percent;
		this.checking = checking;
		...
		for (int y = 0; y < h; y += 1) {
			for (int x = 0; x < w; x += 1) {
				...
				if (rgb != 255) {
					...
					double sum = Math.PI * Math.pow(rad, 2);
					boolean bit = true;
					int count = 0;
					out:for (int i = minX; i < maxX; i++) {
						for (int k = minY; k < maxY; k++) {
							if (Math.sqrt(Math.pow(y - k, 2) + Math.pow(x - i, 2)) < rad) {
								...
								if (sum / (Math.PI * Math.pow(rad, 2)) < percent) {
									bit = false;
									if(!arcOnly) {
										break out;
									}
